"""list_on_market.py — register Hermes as a listing on TapMarket (Base Sepolia).

Builder = deployer wallet (lists + receives payouts).
agentSigner = Hermes's address (signs use-attestations).
"""
import os
...
PRICE_PER_USE = 500000          # 0.5 USDC (6 decimals)
...
def main():
    ...
    print(f"Hermes signer: {hermes_addr}")
    print(f"Price/use:     {PRICE_PER_USE} (0.5 USDC)")
    print(f"Expected listingId: {expected_id}")

    tx = market.functions.listAgent(hermes_addr, PRICE_PER_USE, PAYOUT_EID).build_transaction({
    ...
    print(f"Hermes listed as listingId {expected_id}")
    print(f"\nSave this: HERMES_LISTING_ID={expected_id}")
